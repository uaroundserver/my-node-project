...
// === Fallback auth ===
// если есть middleware/auth.js — он используется, иначе берем JWT прямо отсюда
let auth;
try {
  auth = require('../middleware/auth'); // декодирует JWT -> req.user
} catch {
  const jwt = require('jsonwebtoken');
  auth = function fallbackAuth(req, res, next) {
    try {
...
// === Экспорт: инициализация чата ===
function initChat(server, db, app) {
  ...
  // === Дополнительно: статика для вложений ===
